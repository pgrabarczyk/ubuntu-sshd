...
DOCKER_FILES_CLIENT_PATH=$2
echo "DOCKER_FILES_CLIENT_PATH: ${DOCKER_FILES_CLIENT_PATH}"

useradd -rm -d "${USER_HOME}" -s /bin/bash -g root -G sudo -u 1000 "${USERNAME}"
mkdir –p "${USER_HOME}/.ssh"
chmod 0700 "${USER_HOME}/.ssh"
touch "${USER_HOME}/.ssh/authorized_keys"
cp "${DOCKER_FILES_CLIENT_PATH}/docker_key.pub" "${USER_HOME}/.ssh/authorized_keys"
